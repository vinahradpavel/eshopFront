...
export function BrandsList() {
    ...
    useEffect(() => {
        try {
            fetch(`/brands?page=${page}&limit=${limit}`).then(it => it.json()).then(it => setBrands(it.brands));
        }
        catch (error) {
            console.log(error);
        }
    ...
    useEffect(() => {
        try {
            if (name) {
                fetch(`/brands?page=${page}&limit=${limit}&name=${name}`).then(it => it.json()).then(it => setBrands(it.products));
            }
        }
        catch (error) {
            console.log(error);
